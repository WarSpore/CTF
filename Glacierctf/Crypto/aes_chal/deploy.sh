...
build_container() {
  ...
  docker build -t localhost/chall-aes_chal --platform linux/amd64 . 
}
...
run_container() {
  ...
  docker run --name chall-aes_chal --rm -p 127.0.0.1:1337:1337 -t -i -e HOST=127.0.0.1 -e PORT=1337 -e TIMEOUT=50 --read-only --privileged --platform linux/amd64 localhost/chall-aes_chal
}

kill_container() {
	docker ps --filter "name=chall-aes_chal" --format "{{.ID}}" \
		| tr '\n' ' ' \
